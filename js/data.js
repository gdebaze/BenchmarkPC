...
// Pixel count matters for dual-screen-width panels; coefficient remains indicative.
function effectiveResolutionFactor(resolution,monitor){
 const category=resolutions.find(r=>r.id===resolution);if(!category)return 1;
 const nominal={'1080p':[1920,1080],'1440p':[2560,1440],UWQHD:[3440,1440],'4K':[3840,2160]}[resolution];
 const native=monitor?.native||nominal;
 return nominal&&native&&native.length===2&&native.every(n=>Number.isFinite(n)&&n>0)?category.factor*Math.pow(nominal[0]*nominal[1]/(native[0]*native[1]),.75):category.factor;
}
const monitors={
 '1080p':[
 ...
 'UWQHD':[
  ...
  {id:'oledg949',name:'Samsung Odyssey OLED G9 G95SD',size:'49″',hz:240,panel:'OLED 1800R',price:'≈ 1 500 €',native:[5120,1440],badge:'Dual QHD 240 Hz',img:LOCAL_MONITOR_IMAGES.oledG9_49,url:'https://www.samsung.com/fr/monitors/gaming/odyssey-oled-g9-g95sd-49-inch-oled-dual-qhd-ls49dg954suxen/'}],
 '4K':[
  ...
  {id:'neog957',name:'Samsung Odyssey Neo G9 G95NC',size:'57″',hz:240,panel:'VA Mini LED',price:'≈ 2 000 €',native:[7680,2160],badge:'Dual UHD 240 Hz',img:LOCAL_MONITOR_IMAGES.neoG9_57_front,img2:LOCAL_MONITOR_IMAGES.neoG9_57_angle,url:'https://www.fnac.com/mp50526144/Ecran-LCD-Samsung-S57CG954NU-57-silver/w-4'}]
};
